 ...
 */
export const EVALUATED_SIGNALS: { key: string; label: string }[] = [
  { key: 'score.final.score',  label: 'Final' },
  { key: 'score.factor.score', label: 'Factor' },
  { key: 'score.factor.raw',   label: 'Factor (raw)' },
  { key: 'score.narrative.score', label: 'Narrative' },
  ...PILLAR_KEYS.map((p) => ({ key: `score.factor.pillars.${p}.score`, label: `  ${p}` })),
  { key: 'verdict.score',      label: 'Old LLM score' },
];
...
export async function runEvaluation(opts: {
  ...
  // Sequential and small: Yahoo throttles bursts, and this runs by hand.
  for (const symbol of symbols) {
